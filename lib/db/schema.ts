...
//
// This file is the single source of truth for the shape of the domain
// entities described in the Dayflow requirements: users, employees,
// departments, attendance, leave requests, leave balances, payroll,
// documents, notifications and activity/audit events.
//
// Table creation itself happens via the plain-SQL migration in
// `migrations.sql` (see db/client.ts) — this file exists to give the
// application layer typed, composable query building via drizzle-orm.
...
// ---------------------------------------------------------------------------
// Enums (stored as TEXT with app-level validation; sqlite has no native enum)
// ---------------------------------------------------------------------------
export const ROLES = ["EMPLOYEE", "HR"] as const;
...
export const PAYROLL_STATUSES = ["DRAFT", "PUBLISHED"] as const;
...
export const users = sqliteTable("users", {
  id: text("id").primaryKey(),
  employeeCode: text("employee_code").notNull(), // human-facing Employee ID, e.g. EMP1001
  ...
  employeeCodeIdx: uniqueIndex("users_employee_code_idx").on(t.employeeCode),
}));

export const employees = sqliteTable("employees", {
  id: text("id").primaryKey(), // == users.id (1:1)
  ...
  dateOfJoining: text("date_of_joining"), // YYYY-MM-DD
  phone: text("phone"),
  ...
  userIdx: uniqueIndex("employees_user_id_idx").on(t.userId),
...
export const attendance = sqliteTable("attendance", {
  ...
  checkInAt: text("check_in_at"), // ISO datetime
  checkOutAt: text("check_out_at"), // ISO datetime
  status: text("status").$type<AttendanceStatus>().notNull(),
  notes: text("notes"),
  createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`),
  updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`),
}, (t) => ({
  empDateIdx: uniqueIndex("attendance_employee_date_idx").on(t.employeeId, t.date),
...
export const leaveBalances = sqliteTable("leave_balances", {
  ...
  empTypeYearIdx: uniqueIndex("leave_balances_emp_type_year_idx").on(t.employeeId, t.leaveType, t.year),
}));
...
export const leaveRequests = sqliteTable("leave_requests", {
  ...
  empIdx: index("leave_requests_employee_idx").on(t.employeeId),
...
export const payrollRecords = sqliteTable("payroll_records", {
  ...
  empMonthIdx: uniqueIndex("payroll_employee_month_idx").on(t.employeeId, t.effectiveMonth),
}));
...
export const documents = sqliteTable("documents", {
  ...
  storageKey: text("storage_key").notNull(), // relative path under private storage dir
  uploadedBy: text("uploaded_by").notNull(),
  createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`),
}, (t) => ({
  empIdx: index("documents_employee_idx").on(t.employeeId),
}));
...
export const activityEvents = sqliteTable("activity_events", {
  ...
  action: text("action").notNull(), // e.g. LEAVE_APPROVED, PROFILE_UPDATED
  entityType: text("entity_type").notNull(), // e.g. "leave_request"
  entityId: text("entity_id").notNull(),
  subjectEmployeeId: text("subject_employee_id").notNull(), // whose record this event is about
  metadata: text("metadata"), // JSON string, non-sensitive only
  createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`),
}, (t) => ({
  subjectIdx: index("activity_events_subject_idx").on(t.subjectEmployeeId),
