...
-- Applied idempotently at startup by db/client.ts via `CREATE TABLE IF NOT EXISTS`.
...
CREATE UNIQUE INDEX IF NOT EXISTS users_employee_code_idx ON users(employee_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS employees_user_id_idx ON employees(user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS leave_balances_emp_type_year_idx ON leave_balances(employee_id, leave_type, year);
...
CREATE UNIQUE INDEX IF NOT EXISTS payroll_employee_month_idx ON payroll_records(employee_id, effective_month);
...
CREATE INDEX IF NOT EXISTS activity_events_subject_idx ON activity_events(subject_employee_id);
